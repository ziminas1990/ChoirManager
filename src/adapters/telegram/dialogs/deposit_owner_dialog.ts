    ...
    transactions_info(transactions: Transaction[] | undefined, lang: Language): string {
        ...
        for (const tx of transactions) {
            let lblBalance;
                switch (tx.type) {
                    case "balance":
                        lblBalance = lang === Language.RU ? "Депозит" : "Deposit";
                        break;
                    case "membership":
                        const month = tx.membership_month!.getMonth() + 1;
                        const year = tx.membership_month!.getFullYear() % 100;
                        lblBalance = lang === Language.RU ? `Членский за ${month}/${year}` : `Membership for ${month}/${year}`;
                        break;
                }
            ...
            lines.push(`${dateStr} | ${lblBalance}: ${tx.before} -> ${tx.after} (${sign} ${amount} ${currency})`);
        }
